...
import { AUTH_SECRET, API_DOMAIN, BASE_DOMAIN, MAX_OWNER_DEPLOYMENTS } from "../config";
import * as strings from "../strings";
import kubeClient from "../kube";
import ChallengeConfigStore from "../lib/ChallengeConfigStore";
...
export default async function register(fastify: FastifyInstance) {
  ...
  fastify.addHook('preHandler', async (request, reply) => {
    ...
    try {
      ...
      request.log.info(`auth_info: owner_id=${payload.owner_id}${payload.admin ? ', admin' : ''}`);
    } catch (e) {
      request.log.info(`auth error: ${e.message}`);
      reply.code(401).send({
        error: strings.ERROR_INVALID_CREDENTIALS
      });
    }
  ...
  fastify.route({
    ...
    handler: async (request, reply) => {
      ...
      reply.send({
        deployments: (await kubeClient.getDeploymentsByOwner(owner)).map(mapDeploymentToResponse)
      });
    }
  });
  ...
  fastify.route<{Params: {name: string}}>({
    ...
    schema: {
      params: {
        name: { $ref: 'challenge-manager.downunderctf.com/schema/deployments.json#/properties/name' }
      }
    },
    handler: async (request, reply) => {
      const owner = request.requestContext.get('owner') as string;
      ...
      // Get the deployment
      const deployment = await kubeClient.getDeploymentByNameAndOwner(name, owner);
  ...
  fastify.route<{Params: {name: string}, Body: {reset: boolean, extend: boolean}}>({
    method: 'POST',
    url: '/:name',
    schema: {
      body: {
    ...
    handler: async (request, reply) => {
      ...
      const deployment = deployments.find(({ metadata }) =>
        metadata?.labels?.challenge === request.params.name);
      ...
      if (!spec) return reply.code(400).send({
      ...
      if (deployment && request.body.reset) {
        ...
        if (!admin && MAX_OWNER_DEPLOYMENTS != 0 && deployments.length >= MAX_OWNER_DEPLOYMENTS) {
      ...
      if (!newDeployment) return reply.code(500).send({
  ...
  fastify.route<{Params: {name: string}}>({
    ...
    handler: async (request, reply) => {
      ...
      const spec = await ChallengeConfigStore.getChallenge(request.params.name, true);
      if (!spec) return reply.code(404).send({
        error: strings.ERROR_CHALLENGE_NOT_FOUND
      });
